...
fn compile_assembly(contents: &String) -> Result<Vec<u8>, AnonymousEvaluationError> {
...

    {
        let stdin = child.stdin.as_mut().ok_or(
            AnonymousEvaluationError::new("failed to communicate with gcc process".to_string()))?;
...

    {
        ...
        if !output.status.success() {
            return Err(AnonymousEvaluationError::new("compilation of assembly failed".to_string()))
        }
    }

    let binary = elf::File::open_path(file.path()).map_err(|e|
        AnonymousEvaluationError::new(format!("failed to open elf file: {:?}", e)))?;
    match binary.get_section(".text") {
        Some(section) => Ok(section.data.clone()),
        None => Err(AnonymousEvaluationError::new("failed to find .text in elf file".to_string())),
    }
}
...
impl Block for AssemblyBlock {
    fn evaluate(&self, _: &mut Evaluator) -> Result<Vec<u8>, EvaluationError> {
